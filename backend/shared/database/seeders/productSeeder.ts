...
export async function seedProducts(): Promise<any[]> {
  try {
    // Check if products already exist
    const existingProductsCount = await Product.countDocuments();
    if (existingProductsCount > 0) {
      logger.info(`Found ${existingProductsCount} existing products, skipping product seeding`);
    ...
    for (const productData of sampleProducts) {
      try {
        ...
        logger.info(`Created product: ${productData.name} (${productData.category})`);
      } catch (error) {
...
export async function seedProductsByCategory(category: string, count: number = 10): Promise<any[]> {
  try {
    ...
    for (const productData of categoryProducts) {
      ...
      logger.info(`Created ${category} product: ${productData.name}`);
    }

    return createdProducts;
  } catch (error) {
    logger.error(`Failed to seed products for category ${category}:`, error);
    throw error;
  }
}
...
export async function createTestProduct(overrides: Partial<any> = {}): Promise<any> {
  try {
    ...
    logger.info(`Created test product: ${defaultProduct.name}`);
    return savedProduct;
  } catch (error) {
